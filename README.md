...
* добавляем $f$ в сумму Цекендорфа;#вычитаем $f$ из $x$.
...
в последовательности чисел, отсортированном по возрастанию, методом деления
...
## 1.13 Конкатенация строк (`concat`)
...
в памяти структурой `Elem`, которая объявлена в заголовочном файле `elem.h`
следующим образом:
...
$0 < \le 24$, и каждое целое число находится в диапазоне
от $-10^6$ до $10^6$.
...
## 2.5 Сортировка подсчётом сравнений (`сsort.c`)
...
ввода, содержит размер последовательности $n$ $(0 < n \le 1000000)$. Во второй
...
ввода, содержит строку размера $n$ $(0 < n \le 1000000)$. Строка состоит
...
**Формат входных данных.** Первая строка, считываемая со стандартного потока
ввода, содержит размер последовательности $n$ $(0 < n \le 300000)$. Во второй
строке перечислены элементы последовательности. Каждый элемент представляет
собой целое число, находящееся в диапазоне от $-10^9$ до $10^9$. Элементы
разделяются пробелами.

Третья строка содержит общее количество запросов $m$ $(0 < m \le 1000000)$.
...
**Формат входных данных.** Первая строка, считываемая со стандартного потока
ввода, содержит размер последовательности $n$ $(0 < n \le 1000000)$. Во второй
строке перечислены элементы последовательности. Каждый элемент записывается
в виде $a∕b$, где $a$ и $b$ — неотрицательные целые числа
$(0 \le a \le 1000000, 0 < b \le 1000000)$. Элементы разделяются пробелами.
...
**Формат входных данных.** Первая строка, считываемая со стандартного потока
ввода, содержит общее количество выполняемых операций $n$ $(0 < n \le 100000)$.
...
$-2000000000 < x < 2000000000$), либо форму `DEQ` (удалить головной элемент
из очереди), либо форму `EMPTY` (проверить пустоту очереди).
...
Операция либо имеет форму `ENQ` $x$ (добавить число $x$ в хвост очереди,
$-2000000000 < x < 2000000000$), либо форму `DEQ` (удалить головной элемент
из очереди), либо форму `MAX` (показать текущее максимальное число), либо форму
...
составленная из латинских букв; $-1000000000 < k < 1000000000$, $len(v) < 10$),
либо форму `LOOKUP` $k$ (вывести строку, связанную с ключом $k$), либо форму
`DELETE` $k$ (удалить строку, связанную с ключом $k$), либо форму `RANK` $k$
(вывести порядковый номер словарной пары с ключом $k$).
...
из латинских букв; $-1000000000 < k < 1000000000$, $len(v) < 10$), либо форму
`LOOKUP` $k$ (вывести строку, связанную с ключом $k$), либо форму `DELETE` $k$
(удалить строку, связанную с ключом $k$), либо форму `SEARCH` $x$ (вывести
строку, связанную с ключом, имеющим порядковый номер $x$).
...
## 3.11 Разреженный массив (`disparray.c`)
...
разреженного массива с индексом $i$; $0 \le i < 1000000000$; $-1000000000 <
v < 1000000000)$, либо форму `AT` $i$ (вывести значение элемента разреженного
...
## 3.12 Количество подпоследовательностей, на которых побитовое ИСКЛЮЧАЮЩЕЕ ИЛИ даёт ноль (`zeroxor.c`)

Пусть дана последовательность из $n$ целых чисел, где $0 < n \le 100000$,
...
len(k) < 100000$), либо форму `DELETE` $k$ (удалить из множества имеющуюся
